...
def generate_ir_from_source(source: str):
    ...
    assert scanner.get_errors() == [], f"Lexical errors: {scanner.get_errors()}"

    parser = Parser(tokens)
    ast = parser.parse()
    assert parser.get_errors() == [], f"Parse errors: {parser.get_errors()}"

    analyzer = SemanticAnalyzer("<input>")
    analyzer.analyze(ast, source)
    assert analyzer.get_errors() == [], f"Semantic errors: {analyzer.get_errors()}"
