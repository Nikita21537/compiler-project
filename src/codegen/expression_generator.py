...
class ExpressionGeneratorMixin:
    ...
    def _array_element_address(self, expr: ArrayAccessExprNode) -> IROperand:
        if not isinstance(expr.array, IdentifierExprNode):
            raise NotImplementedError(
                "IR generation for non-identifier array base is not implemented yet"
            )

        base_name = expr.array.name.lexeme
        base_type = self._safe_type_name(expr.array)
        element_type = self._safe_type_name(expr)

        index_op = self._gen_expr(expr.index)
        element_size = self._type_size(element_type)

        # Создаём временную переменную для адреса
        addr_temp_name = self.current_function.new_temp()
        addr_temp = IROperand(
            IROperandKind.TEMP,
            addr_temp_name,
        ...
        # Вычисляем смещение
        if element_size == 1:
            scaled_index = index_op
        else:
            scaled_name = self.current_function.new_temp()
            ...
            self.current_block.add_instruction(
                IRInstruction(
                    opcode=IROpcode.MUL,
                    dest=scaled_index,
                    args=[index_op, IROperand(IROperandKind.LITERAL, element_size, type_name="int")],
                    comment=f"scale index by {element_size}",
                )
            )

        # Вычисляем адрес: base + offset
        self.current_block.add_instruction(
            IRInstruction(
                opcode=IROpcode.GEP,
                dest=addr_temp,
