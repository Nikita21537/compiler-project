...
class PeepholeOptimizer:
    ...
    def _optimize_once(self, lines: list[str]) -> list[str]:
        ...
        while i < len(lines):
            ...
            # Fold move into compare with zero
            ...
            # Remove dead code after unconditional exit
    ...
    def _parse_mov_const(self, line: str):
        """Parse mov reg, const instruction."""
        stripped = line.strip()
        match = re.fullmatch(
            r"mov\s+([a-z0-9]+),\s*(-?\d+)",
            stripped,
