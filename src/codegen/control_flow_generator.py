...
class ControlFlowGeneratorMixin:
    ...
    def _gen_condition_jump(self, expr, true_label: str, false_label: str) -> None:
        ...
        if isinstance(expr, BinaryExprNode) and expr.operator.lexeme == "&&":
            # For &&: jump to false_label if left is false
        ...
        if isinstance(expr, BinaryExprNode) and expr.operator.lexeme == "||":
            # For ||: jump to true_label if left is true
            # Otherwise evaluate right
        ...
        # Default: evaluate expression and jump based on boolean value
    ...
    def _gen_for(self, stmt: ForStmtNode) -> None:
        ...
        # Initialization executes in current block
