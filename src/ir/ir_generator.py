...
from src.ir.ir_instructions import (
    IROpcode, IROperand, IROperandKind, IRInstruction,
...
class IRGenerator:
    ...
    def _gen_stmt(self, stmt) -> None:
        ...
        if isinstance(stmt, VarDeclStmtNode):
            ...
            raise NotImplementedError(
                f"IR generation for statement {type(stmt).__name__} is not implemented"
            )

    def _gen_return(self, stmt: ReturnStmtNode) -> None:
    ...
    def _gen_var_decl(self, stmt: VarDeclStmtNode) -> None:
        ...
        var_op = var(var_name, type_name=type_name)
    ...
    def _gen_if(self, stmt: IfStmtNode) -> None:
        ...
        if has_else:
            ...
            # Try to insert PHI node for value merging
    ...
    def _try_insert_phi(
        ...
        phi_dest = temp(phi_temp_name, type_name=then_value.type_name or else_value.type_name or "unknown")


        phi_arg_1 = lit(f"({then_value.value}, {then_block.label})", type_name=then_value.type_name)
        phi_arg_2 = lit(f"({else_value.value}, {else_block.label})", type_name=else_value.type_name)
        ...
        end_block.add_instruction(
            IRInstruction(
                opcode=IROpcode.STORE,
                args=[
                    var(then_name, type_name=then_value.type_name or else_value.type_name or "unknown"),
                    phi_dest,
                ],
    ...
    def _gen_expr(self, expr) -> IROperand:
        ...
        if isinstance(expr, LiteralExprNode):
            return lit(expr.value, type_name=self._safe_type_name(expr))


        if isinstance(expr, IdentifierExprNode):
            temp_name = self.current_function.new_temp()
            dest = temp(temp_name, type_name=self._safe_type_name(expr))
            src = var(expr.name.lexeme, type_name=self._safe_type_name(expr))
        ...
        if isinstance(expr, UnaryExprNode):
            ...
            if operator in {"++", "--"}:
                ...
                var_op = var(var_name, type_name=self._safe_type_name(expr.operand))


                loaded_name = self.current_function.new_temp()
                loaded = temp(loaded_name, type_name=self._safe_type_name(expr.operand))
                self.current_block.add_instruction(
                    IRInstruction(
                        opcode=IROpcode.LOAD,
                        dest=loaded,
                ...
                result = temp(result_name, type_name=self._safe_type_name(expr))
                op = IROpcode.ADD if operator == "++" else IROpcode.SUB
                self.current_block.add_instruction(
                    IRInstruction(
                        opcode=op,
                        dest=result,
                        args=[loaded, lit(1, type_name="int")],
            ...
            raise NotImplementedError(
                f"IR generation for unary operator {operator!r} is not implemented"
            )
        ...
        if isinstance(expr, AssignmentExprNode):
            ...
            if isinstance(expr.target, IdentifierExprNode):
                ...
                target_op = var(target_name, type_name=target_type)
            ...
            if operator in compound_map:
                ...
                loaded_name = self.current_function.new_temp()
                loaded = temp(loaded_name, type_name=target_type if target_type else "int")
                self.current_block.add_instruction(
                    IRInstruction(
                        opcode=IROpcode.LOAD,
                        dest=loaded,
            ...
            raise NotImplementedError(
                f"IR generation for assignment operator {operator!r} is not implemented"
            )
        ...
        if isinstance(expr, CallExprNode):
            if not isinstance(expr.callee, IdentifierExprNode):
                raise NotImplementedError(
                    "IR generation for non-identifier call targets is not implemented"
                )
            ...
            for i, arg in enumerate(expr.arguments):
                ...
                self.current_block.add_instruction(
                    IRInstruction(
                        IROpcode.PARAM,
                        args=[lit(i, type_name="int"), arg_op],
                        comment=f"param {i}",
                    )
                )

            func_name = expr.callee.name.lexeme
            func_op = var(func_name, type_name="function")
            ...
            if result_type == "void":
                ...
                return lit(None, type_name="void")

            result_name = self.current_function.new_temp()
            result_temp = temp(result_name, type_name=result_type)
        ...
        raise NotImplementedError(
            f"IR generation for expression {type(expr).__name__} is not implemented"
        )
    ...
    def _struct_field_address(self, expr: StructAccessExprNode) -> IROperand:
        ...
        if not isinstance(expr.primary, IdentifierExprNode):
            raise NotImplementedError(
                "IR generation for nested/non-identifier struct access is not implemented"
            )
        ...
        addr_temp = temp(addr_temp_name, type_name=f"{field_type}*")

        base_operand = var(base_name, type_name=base_type)
        offset_operand = lit(offset, type_name="int")
        ...
        return mem(addr_temp.value, type_name=field_type)
