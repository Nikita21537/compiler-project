...
class ConstantPropagationPass:
    ...
    def _rewrite_instruction(self, instr, constants):
        ...
        if instr.opcode in {IROpcode.ALLOCA, IROpcode.GEP,
                           IROpcode.ADDR_OF, IROpcode.MEMCPY,
                           IROpcode.PARAM}:
            return
    ...
    def _rewrite_load(self, instr, constants):
        if (instr.dest is None or len(instr.args) != 1 or
            instr.args[0].kind != IROperandKind.VARIABLE):
            return
    ...
    def _track_constants(self, instr, constants):
        if (instr.opcode == IROpcode.STORE and len(instr.args) == 2 and
            instr.args[0].kind == IROperandKind.VARIABLE):
            target = instr.args[0]
            value = instr.args[1]
        ...
        if (instr.opcode == IROpcode.MOVE and
            instr.dest is not None and
            instr.dest.kind == IROperandKind.TEMP and
            instr.args and
            instr.args[0].kind == IROperandKind.LITERAL):
